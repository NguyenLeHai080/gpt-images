...
export const ModelDistributionCard: React.FC<ModelDistributionCardProps> = ({
  ...
  const totalReqs = models.reduce((acc, m) => acc + (m.request_count || 0), 0) || 1;
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col justify-between h-full gap-4">
      ...
      <div className="space-y-4 flex-1">
        {models.map((m) => {
          const pct = Math.round(((m.request_count || 0) / totalReqs) * 100);
          return (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400 block font-mono">
                      {m.model_name.includes('2.5') ? 'Thế Hệ 2.5 Tiên Tiến' : 'Model Tiêu Chuẩn'}
                    </span>
                  </div>
                </div>
      ...
      <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
        ...
        <span className="font-bold text-slate-800">
          {totalReqs.toLocaleString()} Requests ({models.length} Models)
        </span>
      </div>
    </div>
  );
