...
import { AlertTriangle, CheckCircle2, DollarSign, Plus, Zap } from 'lucide-react';
...
interface CreateModelPricingModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (newItem: ModelPricingItem) => void;
}

export const CreateModelPricingModal: React.FC<CreateModelPricingModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const [provider, setProvider] = useState('Xompet AI Gateway');
  ...
  const [latencyRange, setLatencyRange] = useState('25ms (Cache) - 3.2s (Gen)');
  ...
  // Projection for 10,000 requests (35% smart cache)
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!cleanModel) {
      alert.toast('Vui lòng nhập mã định danh model (vd: gpt-image-3)', 'warning');
      return;
    }
    if (!displayName.trim()) {
      alert.toast('Vui lòng nhập tên hiển thị model', 'warning');
      return;
    }
    if (basePrice <= 0) {
    ...
    if (providerCost < 0) {
      alert.toast('Chi phí vốn nhà cung cấp không hợp lệ', 'warning');
      return;
    }
    ...
    if (isLoss) {
      const confirmProceed = await alert.confirm({
        ...
        text: `Giá bán (${basePrice.toLocaleString()}đ) đang THẤP HƠN chi phí NCC (${providerCost.toLocaleString()}đ). Bạn có chắc chắn muốn tạo model với mức giá lỗ này?`,
        confirmButtonText: 'Vẫn tạo model',
    ...
    try {
      ...
      if (res.success && res.data) {
        alert.toast(`Đã thêm mới model ${res.data.display_name} thành công!`, 'success');
        onSuccess(res.data);
        onClose();
      } else {
        alert.error('Thêm thất bại', res.message || 'Vui lòng kiểm tra lại');
      }
    } catch (err: any) {
      alert.error('Lỗi tạo model', err?.message || 'Không thể tạo model mới');
    } finally {
  ...
  return (
    <Modal
      ...
      title={
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-brand-500 to-amber-500 text-white flex items-center justify-center shadow-xs">
            <Plus size={18} />
          </div>
      ...
      description="Thiết lập giá vốn NCC, giá thu khách và tỷ lệ token cho mô hình AI mới."
      footer={
        <>
          <Button
            type="button"
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="pricing-form-group">
            <div className="pricing-form-group__header">
              <label className="pricing-form-group__label">
                Mã định danh model <span className="pricing-form-group__required">*</span>
              </label>
            </div>
            <div className="pricing-form-group__control">
              <input
                ...
                placeholder="vd: gpt-image-3, flux-pro..."
                className="pricing-form-group__input font-mono"
              />
            </div>
            <span className="pricing-form-group__hint">
              Dùng trong API payload (viết liền không dấu)
            </span>
          </div>
          ...
          <div className="pricing-form-group">
            ...
            <div className="pricing-form-group__control">
              <input
                ...
                placeholder="vd: GPT Image 3 Turbo"
                className="pricing-form-group__input"
              />
            </div>
            <span className="pricing-form-group__hint">Hiển thị trên bảng giá và giao diện</span>
          </div>
        </div>
        ...
        {/* Financial Numbers: Provider Cost, Base Price, Token Rate, Status */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="pricing-form-group">
          ...
          <div className="pricing-form-group__control">
            <input
              ...
              placeholder="VD: Mô hình thế hệ mới với khả năng hiểu prompt xuất sắc..."
              className="pricing-form-group__input"
            />
          </div>
        </div>
