...
class ApiKeyService:
    @staticmethod
    def list_keys(user_id: Optional[str] = None, is_admin: bool = False, db: Optional[Session] = None) -> List[ApiKeyItem]:
        close_session = False
        if db is None:
            db = SessionLocal()
            close_session = True
        ...
        try:
            ...
            if not is_admin and user_id:
                query = query.filter((ApiKey.user_id == user_id) | (ApiKey.user_id.is_(None)))
    ...
    @staticmethod
    def create_key(payload: CreateApiKeyRequest, user_id: Optional[str] = None, db: Optional[Session] = None) -> ApiKeyItem:
        close_session = False
        if db is None:
            db = SessionLocal()
            close_session = True
